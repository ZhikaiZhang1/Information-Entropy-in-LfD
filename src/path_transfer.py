...
def retrieve_demos(number_of_demos):  # obtain demos
    ...
    data = np.expand_dims(np.load(settings.directory_name + '/demo' + str(settings.starting_index) + '.npy'),
                          axis=2)  # examine demo 1
    settings.data = []
    demo = np.expand_dims(np.load(settings.directory_name + '/demo' + str(settings.starting_index) + '.npy'),
                          axis=2)  # expand dimension
    settings.data.append([demo[99, 1, 0], demo[99, 2, 0]])
    # settings.data.append(data[settings.starting_index,:,:])
    for i in range(settings.starting_index + 1, settings.starting_index + number_of_demos):
...
def plot_demos(number_of_demos):  # visually plot demo entries
    if number_of_demos <= 0:  # ensure number of demos is greater than 0
        print("Must be greater than 0")
        return
    average_starting = np.array([np.mean(settings.Data[99, 1, :]), np.mean(settings.Data[99, 2, :])])
    for i in range(settings.starting_index, settings.starting_index + number_of_demos):
...
def get_frames(data, dims):  # generate frames of observation
    frames = np.array([([[1, 0], [0, 1]], [data[99, 1, 0], data[99, 2, 0]]),  # start point
                       ([[1, 0], [0, 1]], [data[0, 1, 0], data[0, 2, 0]]),  # end point
                       #   , ([[1, 0], [0, 1]], [-0.12 + settings.shift[0], 0.19 + settings.shift[1]]),
                       #([[1, 0], [0, 1]], [0.12 + settings.shift[0], 0.34 + settings.shift[1]])
                       ]  # obstacles
                      , dtype=get_frame_dtype(dims))
    frames = np.expand_dims(frames, axis=1)

    for i in range(1, np.size(data, axis=2)):
        frame_demo = np.array([([[1, 0], [0, 1]], [data[99, 1, i], data[99, 2, i]]),  # start point
                               ([[1, 0], [0, 1]], [data[0, 1, i], data[0, 2, i]]),  # end point
                               #   , ([[1, 0], [0, 1]], [-0.12 + settings.shift[0], 0.19 + settings.shift[1]]),
                               #([[1, 0], [0, 1]], [0.12 + settings.shift[0], 0.34 + settings.shift[1]])
                               ]  # obstacles
                              , dtype=get_frame_dtype(dims))
...
def trajectory_grid(increment, end_goal_radius):  # generate grid of trajectories
    ...
    return (x_points, y_points, point_dict, max_uncertainty_pt, jerk, pnt_dis)
...
def start_end_dis(new_path, x, y):
    start_dis = sqrt((x - new_path['x'][0, 0, -1]) ** 2 + (
                y - new_path['x'][0, 1, -1]) ** 2)  # pythagoras to determine distance away from intended start
    end_dis = sqrt((settings.goal_x - new_path['x'][0, 0, 0]) ** 2 + (
                settings.goal_y - new_path['x'][0, 1, 0]) ** 2)  # pythag to determine distance away from goal
...
def p_controller(x_target, y_target):
    ...
    while abs(x_error) > settings.pd_accuracy or abs(y_error) > settings.pd_accuracy:
        ...
        x_error = x_target - new_path['x'][0, 0, -1]
        y_error = y_target - new_path['x'][0, 1, -1]
...
x_points, y_points, point_dict, max_u_pt, jerk, pnt_distance= trajectory_grid(settings.increment,
